...
namespace TiagoSimulation
{
    ...
    TiagoSimulationSampleComponent::TiagoSimulationSampleComponent()
    {
        m_goalTopicConfiguration.m_topic = "/goal_pose";
        m_goalTopicConfiguration.m_type = "geometry_msgs::msg::PoseStamped";
    }

    void TiagoSimulationSampleComponent::Activate()
    {
    ...
    void TiagoSimulationSampleComponent::Deactivate()
    {
        m_goalPublisher.reset();
        ImGui::ImGuiUpdateListenerBus::Handler::BusDisconnect();
    }

    void TiagoSimulationSampleComponent::Reflect(AZ::ReflectContext* context)
    {
        if (AZ::SerializeContext* serialize = azrtti_cast<AZ::SerializeContext*>(context))
        {
            serialize->Class<TiagoSimulationSampleComponent, AZ::Component>()
                ->Version(1)
                ->Field("goals", &TiagoSimulationSampleComponent::m_goalEntities)
                ->Field("goalMessageTopic", &TiagoSimulationSampleComponent::m_goalTopicConfiguration);

            if (AZ::EditContext* ec = serialize->GetEditContext())
            {
                ec->Class<TiagoSimulationSampleComponent>("TiagoSimulationSampleComponent", "A sample component that sends goal")
                    ->ClassElement(AZ::Edit::ClassElements::EditorData, "TiagoSimulationSampleComponent")
                    ->Attribute(AZ::Edit::Attributes::AppearsInAddComponentMenu, AZ_CRC("Game"))
                    ->Attribute(AZ::Edit::Attributes::Category, "Sample ROS2 Project")
                    ->DataElement(AZ::Edit::UIHandlers::Default, &TiagoSimulationSampleComponent::m_goalEntities, "Goals", "Entities goals")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &TiagoSimulationSampleComponent::m_goalTopicConfiguration,
    ...
    void TiagoSimulationSampleComponent::OnImGuiUpdate()
    {
        ImGui::Begin("TiagoSimulationSampleComponent");
